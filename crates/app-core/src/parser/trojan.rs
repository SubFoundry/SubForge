...
pub(crate) fn parse_trojan_uri(
    ...
    let transport = map_transport(query_value(&query_pairs, &["type"]));
    ...
    insert_optional_string(&mut extra, "security", security.clone());
    ...
    Ok(build_proxy_node(
        ...
        transport,
        TlsConfig {
            enabled: !matches!(security.as_deref(), Some("none")),
            server_name: sni,
        },
        extra,
        updated_at,
