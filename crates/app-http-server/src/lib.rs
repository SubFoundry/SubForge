...
mod tests {
    ...
    async fn e2e_import_source_refresh_and_raw_profile_output() {
        ...
        let import_body = build_multipart_plugin_body(boundary, &plugin_zip);
    ...
    fn build_multipart_plugin_body(boundary: &str, zip_payload: &[u8]) -> Vec<u8> {
        let mut body = Vec::new();
        write!(body, "--{boundary}\r\n").expect("写入 multipart 边界失败");
        write!(
            body,
            "Content-Disposition: form-data; name=\"file\"; filename=\"builtin-static.zip\"\r\n"
        )
